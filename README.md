...
- When using the non-cloud configuration, some basic understanding of your local network

This skill is intended to work as an internal API (leveraging a scene) that can be used to perform actions on a slide. The main API's used by this skill are the ones to request a token, get the information that is applicable to your slide, and manipulating your actual slide (e.g. closing/opening). In case you prefer to only run the devices locally and not pass through the cloud integration, then the main functionality is the ability to open and close the curtains locally.

The basic features supported are:
- performing dry-runs to see if your credentials are okay and your household is properly provisioned (in both cloud and local mode)
- requesting tokens on behalve of your user
- opening/closing your slide
- using a slider to set custom values for open and close (e.g. partially open)
...
```
When you want to use the cloud version of the API the userdata has been set and you want to  get going, head over to devices and import the example VFIB file (any VFIB that does not have a reference to local in the name) to get the basics behind the way in which the virtual device calls the scene and how the device itself is dynamically provisioned. For this to work it's important to do a dry run on the scene (instructions below) and to create three global variables in the panel section of Fibaro. Just create blank ones for the first two - they will be filled by the scene- but insert the scene id for the newly create code in the variable for slidescene. So create scene, remember scene id, and then insert that scene id in the value when creating a global variable called Slidescene

The following variables needs to be created to comply to the scene's storing syntax when using the cloud API's:
- Slidetoken (Mandatory when running in cloud)
- Slides (Mandatory when running in cloud)
- Slidescene (Always mandatory)
...
slideIdToTest = 1867           -- insert the slideID (the number can be found in the household call OR is the IP at which SLIDE is reachable on the local network)
...
Create the following global variables depending on your desired implementaion (cloud versus non-cloud):
- Slidetoken (Only required when running in cloud)
- Slides (Only required when running in cloud)
- Slidescene (Mandatory at all times)
...
Option 1 - Creating a household based virtual device for all slides leveraging the cloud API
...
Option 2 - creating individual slides levearging the cloud API
...
Option 3 - creating individual slides by creating local network devices
- import vfib with the name Single_Slide_Local.vfib in the virtual device section
- keep the code of the labels as is
- for every device it's important to validate and change the IP on which the slide can be reached
- in the advanced section of the virtual device make sure you list the IP at which the Slide can be reached
- Insert the artwork for the individual states of the slide
- You can try to call a single slide based on the entered IP
