...
function getFibaroToken()
    debuglogger("Calling FibaroToken function to get the right Token for Slide")
    -- decoded global value for slidetoken
    if dryrun then
        fibaro:debug("Dry-run active, using data from local variable instead of global Slidetoken variable")
        decodedGlobalToken = authData
        return decodedGlobalToken
    elseif dryrun == false and fibaro:getGlobalValue("Slidetoken") == "NaN" then
        fibaro:debug("Warning, token data is corrupt recreate Slidetoken variable or set to 0")
        return
    elseif
     ...
     then
        fibaro:debug("Retrieving global Fibaro variable Slidetoken to run authentication")
        retrieveToken = fibaro:getGlobalValue("Slidetoken")
        decodedGlobalToken = json.decode(retrieveToken)
        return decodedGlobalToken
    else
        fibaro:debug("Warning, global value for Slidetoken is not set or 0")
        return
    end
end
